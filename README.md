...
hosting, a screenshot of the result, and this repo — from vendors it had no account with — paid per call
through the [Nevermined Router](https://nevermined.ai), on a budget a human capped in advance.
...
- `receipt/receipt.json` — the curated receipt table rendered on the site: one row per settled payment,
  with amounts, the Router's 2% fee shown separately, and the on-chain transaction hash for each.
...
purchase — hosting and a domain from Locus, a screenshot from ScreenshotOne — it called the Nevermined
